"""
Incremental Directory Indexer for Cicada Vector.
Handles file scanning, hashing, and incremental updates.
"""

import hashlib
import json
import os
import sys
from pathlib import Path
from typing import Set, Dict, List, Tuple, Optional

from .rag import VectorIndex
from .embeddings import EmbeddingProvider

DEFAULT_EXTENSIONS = {'.py', '.ex', '.exs', '.md', '.json', '.toml', '.sh', '.rs', '.go', '.js', '.ts', '.tsx'}
DEFAULT_EXCLUDE = {'.git', '.venv', '__pycache__', 'node_modules', '.pytest_cache', '.gemini', 'target', 'dist', 'build'}

BATCH_SIZE = 8  # Number of files to embed in a single Ollama call


class DirectoryIndexer:
    def __init__(
        self,
        storage_dir: str,
        embedding_provider: Optional[EmbeddingProvider] = None,
        ollama_host: str = "http://localhost:11434",
        ollama_model: str = "nomic-embed-text"
    ):
        """
        Initialize directory indexer.

        Args:
            storage_dir: Directory for storage files
            embedding_provider: Custom embedding provider (if None, uses Ollama)
            ollama_host: Ollama host (used if embedding_provider is None)
            ollama_model: Ollama model (used if embedding_provider is None)
        """
        self.storage_dir = Path(storage_dir)
        self.rag_db = VectorIndex(
            str(storage_dir),
            embedding_provider=embedding_provider,
            ollama_host=ollama_host,
            ollama_model=ollama_model
        )

        self.hashes_path = self.storage_dir / "file_hashes.json"
        self.hashes: Dict[str, str] = {}
        self._load_hashes()

    def _load_hashes(self):
        if self.hashes_path.exists():
            try:
                with open(self.hashes_path, 'r') as f:
                    self.hashes = json.load(f)
            except Exception:
                self.hashes = {}

    def _save_hashes(self):
        with open(self.hashes_path, 'w') as f:
            json.dump(self.hashes, f, indent=2)

    def _compute_hash(self, content: str) -> str:
        return hashlib.md5(content.encode('utf-8')).hexdigest()

    def index_directory(
        self,
        root_path: str,
        extensions: Set[str] = DEFAULT_EXTENSIONS,
        exclude: Set[str] = DEFAULT_EXCLUDE,
        verbose: bool = False
    ) -> Dict[str, int]:
        """
        Incrementally index a directory.
        Returns stats: {'added': int, 'skipped': int, 'failed': int}
        """
        root = Path(root_path).resolve()
        if not root.exists():
            raise FileNotFoundError(f"Path {root} does not exist")

        stats = {'added': 0, 'skipped': 0, 'failed': 0}

        # Phase 1: Scan files and collect those that need indexing
        all_files = []
        for p in root.rglob("*"):
            if p.is_file() and p.suffix in extensions:
                if any(part in exclude for part in p.parts):
                    continue
                if str(self.storage_dir.name) in p.parts:
                    continue
                all_files.append(p)

        if verbose:
            print(f"Scanning {len(all_files)} files...", file=sys.stderr)

        pending: List[Tuple[str, str]] = []  # (rel_path, content)

        for file_path in all_files:
            rel_path = str(file_path.relative_to(root))
            try:
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()

                if not content.strip():
                    continue

                current_hash = self._compute_hash(content)
                if rel_path in self.hashes and self.hashes[rel_path] == current_hash:
                    stats['skipped'] += 1
                    continue

                pending.append((rel_path, content))

            except Exception as e:
                stats['failed'] += 1
                if verbose:
                    print(f"\nError reading {rel_path}: {e}", file=sys.stderr)

        if verbose and pending:
            print(f"Embedding {len(pending)} new/changed files...", file=sys.stderr)

        # Phase 2: Batch embed and index
        consecutive_failures = 0
        MAX_CONSECUTIVE_FAILURES = 3
        should_stop = False

        for batch_start in range(0, len(pending), BATCH_SIZE):
            if should_stop:
                break

            batch = pending[batch_start:batch_start + BATCH_SIZE]

            if verbose:
                print(
                    f"\r[{batch_start + 1}/{len(pending)}] Embedding {batch[0][0]}...",
                    end="", flush=True, file=sys.stderr
                )

            embed_texts = [self.rag_db.prepare_embed_text(p, c) for p, c in batch]

            try:
                vectors = self.rag_db.db.embedder.embed_batch(embed_texts)
                for (rel_path, content), vector in zip(batch, vectors):
                    self.rag_db.add_file(rel_path, content, vector=vector)
                    self.hashes[rel_path] = self._compute_hash(content)
                    stats['added'] += 1
                consecutive_failures = 0

            except RuntimeError as batch_err:
                # Batch failed — fall back to one-by-one to isolate the problem file
                if verbose:
                    print(f"\nBatch embed failed ({batch_err}), retrying one by one...", file=sys.stderr)

                for rel_path, content in batch:
                    try:
                        self.rag_db.add_file(rel_path, content)
                        self.hashes[rel_path] = self._compute_hash(content)
                        stats['added'] += 1
                        consecutive_failures = 0
                    except Exception as file_err:
                        stats['failed'] += 1
                        consecutive_failures += 1

                        if verbose:
                            print(f"\nError indexing {rel_path}: {file_err}", file=sys.stderr)

                        if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                            error_msg = f"\nStopped after {consecutive_failures} consecutive embedding failures.\n"
                            if "500" in str(file_err) or "Internal Server Error" in str(file_err):
                                error_msg += "Ollama returned HTTP 500. Try: ollama pull nomic-embed-text\n"
                            elif "Connection" in str(file_err) or "refused" in str(file_err):
                                error_msg += "Cannot connect to Ollama. Is it running? (ollama serve)\n"
                            else:
                                error_msg += f"Error: {file_err}\n"
                            print(error_msg, file=sys.stderr)
                            should_stop = True
                            break

        # Finalize
        self.rag_db.persist()
        self._save_hashes()

        if verbose:
            print(
                f"\n\nDone. Added: {stats['added']}, Skipped: {stats['skipped']}, Failed: {stats['failed']}",
                file=sys.stderr
            )

        return stats
